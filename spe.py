...
def foo2(N, coords, commits_arr, learning_rate, epsilon):
    ...
    # indices = np.arange(N)
    # indices = indices[:(N//n_threads)*n_threads]
    # indices = indices.reshape(n_threads, -1)

    # for thread_idx in prange(n_threads):
    #     for l in indices[thread_idx]:
    #         if i == l:
    #             continue
    #         xl = coords[l]
    #         dij = np.linalg.norm(xi - xl)
    #         rij = 1 - (jaccard_fast(commits_arr[i], commits_arr[l]))**0.5
    #         # rij = 1 / ((jaccard_fast(commits_arr[i], commits_arr[l])) + epsilon)
    #         # rij = 1 - estimated_jaccard(commits_arr, i, l)**0.5
    #         update_factor = (xi - xl) * learning_rate * \
    #             (rij - dij) / (dij + epsilon)
...
@numba.jit(nopython=True, parallel=True, fastmath=True)
def spe_optimized_parallel_fancy_animated(commits_arr, n_iter, lr, lr_final, num_frames, epsilon=1e-8):
    N = len(commits_arr)
    learning_rate = lr
    lr_initial = lr
    coords = np.random.rand(N, 2) - 0.5
